...
impl PullRequest {
    pub async fn create(
        ...
        let now = Utc::now();
        sqlx::query!(
            "INSERT OR IGNORE INTO pull_requests (id, workspace_id, repo_id, pr_url, pr_number, pr_status, target_branch_name, created_at)
            VALUES (?, ?, ?, ?, ?, 'open', ?, ?)",
            id,
            workspace_id,
            repo_id,
    ...
    pub async fn update_status(
        ...
        let now = Utc::now();
        sqlx::query!(
            "UPDATE pull_requests SET pr_status = ?, merged_at = ?, merge_commit_sha = ?, updated_at = ? WHERE pr_url = ?",
            status_str,
            merged_at,
            merge_commit_sha,
            now,
