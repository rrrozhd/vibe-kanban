...
const LinkPrToIssueDialogImpl = create<LinkPrToIssueDialogProps>(
  ({ issueId }) => {
    ...
    const prInfo = useMemo<PullRequestInfo | null>(() => {
    ...
    const linkMutation = useMutation({
      mutationFn: async () => {
        if (activeTab === 'url') {
          if (!prInfo) throw new Error('Missing PR info');
          const result = await issuePrsApi.linkToIssue({
            issue_id: issueId,
            pr_number: prInfo.number,
            pr_url: prInfo.url,
            base_branch: prInfo.base_branch ?? 'main',
          });
          if (!result.success && result.error) {
            const error = result.error as LinkPrError;
            switch (error.type) {
              case 'not_authenticated':
                throw new Error(t('linkPrToIssue.errors.notAuthenticated'));
              case 'already_linked':
                throw new Error(t('linkPrToIssue.errors.alreadyLinked'));
              case 'remote_error':
                throw new Error(
                  error.message || t('linkPrToIssue.errors.remoteError')
                );
            }
          }
          return result;
        } else {
          if (!selectedPr) throw new Error('Missing selected PR');
          const result = await issuePrsApi.linkToIssue({
            issue_id: issueId,
      ...
      onSuccess: () => {
        queryClient.invalidateQueries({ queryKey: ['issue-prs'] });
    ...
    const canLink =
      activeTab === 'url'
        ? !!prInfo && !linkMutation.isPending
        : !!selectedPr && !linkMutation.isPending && !isLoadingPrs;
            ...
            {activeTab === 'url' && (
              <div className="space-y-3">
                <div className="space-y-2">
                  <Label>
                    {t('linkPrToIssue.urlLabel', 'Pull Request URL')}
                  </Label>
                  <Input
                    ...
                    {isLoadingRemotes ? (
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue
                            placeholder={t(
                              'createWorkspaceFromPr.selectRemote'
                            )}
                          />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  {isLoadingPrs || isLoadingRemotes ? (
                    ...
                    <div className="flex items-center gap-2">
                      <SearchableDropdownContainer
                        ...
                        onSelect={(pr) =>
                          setSelectedPrNumber(Number(pr.number))
                        }
                        trigger={
                          <Button
                            variant="outline"
          ...
          <DialogFooter>
            ...
            <Button onClick={handleLink} disabled={!canLink}>
              {linkMutation.isPending
                ? t('linkPrToIssue.linking')
                : t('linkPrToIssue.linkPr')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
