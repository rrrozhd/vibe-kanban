...
export const issuePrsApi = {
  getPrInfo: async (
    url: string
  ): Promise<Result<PullRequestInfo, ListPrsError>> => {
    const response = await makeRequest(
      `/api/repos/pr-info?url=${encodeURIComponent(url)}`
    );
    return handleApiResponseAsResult<PullRequestInfo, ListPrsError>(response);
  },

  linkToIssue: async (
    data: LinkPrToIssueRequest
  ): Promise<Result<void, LinkPrError>> => {
    const response = await makeRequest('/api/remote/pull-requests/link', {
      method: 'POST',
      body: JSON.stringify(data),
    });
    return handleApiResponseAsResult<void, LinkPrError>(response);
  },
};
