...
impl CreatePullRequestRequest {
    fn all_issue_ids(&self) -> Vec<Uuid> {
        let mut ids = self.issue_ids.clone();
...
async fn create_pull_request(
    ...
    let issue_ids = payload.all_issue_ids();

    if issue_ids.is_empty() {
        return Err(ErrorResponse::new(
            StatusCode::BAD_REQUEST,
            "at least one issue_id is required",
        ));
    }
    ...
    let first_issue = IssueRepository::find_by_id(state.pool(), issue_ids[0])
        .await
        .map_err(|error| {
    ...
    for &issue_id in &issue_ids {
        PullRequestIssueRepository::create(&mut *tx, pr.id, issue_id)
            .await
            .map_err(|error| {
